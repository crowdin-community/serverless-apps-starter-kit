...
- `templates/projects-dashboard-cli` — app code only. The `crowdin-serverless-apps` CLI owns Vite, Lingui, Biome, and the base tsconfig (`extends: @crowdin/serverless-apps-cli/tsconfig.base.json`). **Do not add toolchain config files (vite.config, lingui.config, biome.json) to this template** — their absence is the point.
...
- `src/index.tsx` — entry point: calls `prepareOrganizationMenu(module)` / `prepareProfileResourcesMenu(module)` from the SDK, one per manifest module.
- `src/modules/dashboard/index.tsx` — a `ModuleContract` (`{ render }`) that mounts React into `#root`, wrapped in `AppI18nProvider` + `AppUiProvider`.
- `src/modules/dashboard/app.tsx` — the actual UI.
- `.env` with `CROWDIN_APP_ID` — per-developer link to the registered app, written by `create`/`link`, gitignored.

### SDK surface (`@crowdin/serverless-apps-sdk`)

- root: host-iframe interactions — `prepare*Menu`, `resize()`, `redirect(url)`
- `/api`: `createCrowdinClient()` — pre-authenticated `@crowdin/crowdin-api-client`
- `/react`: `useCrowdinContext()` — detect Crowdin vs Enterprise via its `isEnterprise` flag (never infer edition from module type or URL)
...
- `scripts/build.mjs` runs Vite, then `compile-i18n.mjs` (Vite's `emptyOutDir` would wipe the catalogs), then zips `dist/` into `dist/bundle.zip` deterministically (fixed 1980 mtime — fflate rejects mtime 0).
